...
def create_default_ini():
    ...
    config["DHCP"] = {
        ...
        "dns": ip_prefix + ".1" if "ip_prefix" in locals() else "192.168.1.1",
        "lease_time": "86400",
    }
    config["FileServer"] = {
        "tftp_root": ".",
...
""",
    ...
    config["DHCPOptions"] = {"enabled": "false", "options_text": EXAMPLE_OPTIONS}
...
def load_config_from_ini():
    ...
    try:
        ...
        log_message(f"配置文件 '{INI_FILENAME}' 解析失败: {e}", "ERROR")
        return False
    try:
        g, d, fs, b, s = (
            config["General"],
        ...
        SETTINGS = {
            ...
            "subnet_mask": d.get("subnet"),
            "router_ip": d.get("router"),
            "dns_server_ip": d.get("dns"),
            "lease_time": d.getint("lease_time"),
            "tftp_root": fs.get("tftp_root"),
            "http_root": fs.get("http_root"),
            "smb_root": fs.get("smb_root"),
            "tftp_enabled": fs.getboolean("tftp_enabled"),
            "http_enabled": fs.getboolean("http_enabled"),
            "http_port": fs.getint("http_port"),
            "tftp_multithread": fs.getboolean("tftp_multithread"),
            "tftp_max_transfers": fs.getint("tftp_max_transfers", fallback=60),
            "http_multithread": fs.getboolean("http_multithread"),
            "bootfile_bios": b.get("bios"),
            "bootfile_uefi32": b.get("uefi32"),
            "bootfile_uefi64": b.get("uefi64"),
            "bootfile_ipxe": b.get("ipxe"),
            "smb_enabled": s.getboolean("enabled"),
            "smb_share_name": s.get("share_name"),
            "smb_permissions": s.get("permissions"),
            "pxe_menu_bios_enabled": pm_bios.getboolean("enabled"),
            "pxe_menu_bios_timeout": pm_bios.getint("timeout"),
            "pxe_menu_bios_randomize_timeout": pm_bios.getboolean(
                "randomize_timeout", False
            ),
            "pxe_menu_bios_prompt": pm_bios.get("prompt"),
            "pxe_menu_bios_items": pm_bios.get("items"),
            "pxe_menu_uefi_enabled": pm_uefi.getboolean("enabled"),
            "pxe_menu_uefi_timeout": pm_uefi.getint("timeout"),
            "pxe_menu_uefi_randomize_timeout": pm_uefi.getboolean(
                "randomize_timeout", False
            ),
            ...
            "pxe_menu_ipxe_randomize_timeout": pm_ipxe.getboolean(
                "randomize_timeout", False
            ),
        ...
        log_message(
            f"读取配置文件时发生错误: {e}。请检查 '{INI_FILENAME}' 的格式。", "ERROR"
        )
        return False
...
def save_config_to_ini():
    try:
        g, d, fs, b, s, pm_bios, pm_uefi, pm_ipxe, o = (
            config["General"],
            config["DHCP"],
            config["FileServer"],
            config["BootFiles"],
        ...
        d["pool_start"], d["pool_end"] = (
            SETTINGS["ip_pool_start"],
            SETTINGS["ip_pool_end"],
        )
        d["subnet"], d["router"], d["dns"] = (
            SETTINGS["subnet_mask"],
            SETTINGS["router_ip"],
            SETTINGS["dns_server_ip"],
        )
        d["lease_time"] = str(SETTINGS["lease_time"])
        fs["tftp_root"], fs["http_root"], fs["smb_root"] = (
            SETTINGS["tftp_root"],
            SETTINGS["http_root"],
            SETTINGS["smb_root"],
        )
        fs["tftp_enabled"], fs["http_enabled"], fs["http_port"] = (
            str(SETTINGS["tftp_enabled"]).lower(),
            str(SETTINGS["http_enabled"]).lower(),
            str(SETTINGS["http_port"]),
        )
        fs["tftp_multithread"], fs["tftp_max_transfers"] = (
            str(SETTINGS["tftp_multithread"]).lower(),
            str(SETTINGS["tftp_max_transfers"]),
        )
        fs["http_multithread"] = str(SETTINGS["http_multithread"]).lower()
        b["bios"], b["uefi32"], b["uefi64"], b["ipxe"] = (
            SETTINGS["bootfile_bios"],
            SETTINGS["bootfile_uefi32"],
            SETTINGS["bootfile_uefi64"],
            SETTINGS["bootfile_ipxe"],
        )
        s["enabled"], s["share_name"], s["permissions"] = (
            str(SETTINGS["smb_enabled"]).lower(),
            SETTINGS["smb_share_name"],
            SETTINGS["smb_permissions"],
        )
        pm_bios["enabled"], pm_bios["timeout"] = (
            str(SETTINGS["pxe_menu_bios_enabled"]).lower(),
            str(SETTINGS["pxe_menu_bios_timeout"]),
        )
        pm_bios["randomize_timeout"] = str(
            SETTINGS["pxe_menu_bios_randomize_timeout"]
        ).lower()
        pm_bios["prompt"], pm_bios["items"] = (
            SETTINGS["pxe_menu_bios_prompt"],
            SETTINGS["pxe_menu_bios_items"],
        )
        pm_uefi["enabled"], pm_uefi["timeout"] = (
            str(SETTINGS["pxe_menu_uefi_enabled"]).lower(),
            str(SETTINGS["pxe_menu_uefi_timeout"]),
        )
        pm_uefi["randomize_timeout"] = str(
            SETTINGS["pxe_menu_uefi_randomize_timeout"]
        ).lower()
        pm_uefi["prompt"], pm_uefi["items"] = (
            SETTINGS["pxe_menu_uefi_prompt"],
            SETTINGS["pxe_menu_uefi_items"],
        )
        pm_ipxe["enabled"], pm_ipxe["timeout"] = (
            str(SETTINGS["pxe_menu_ipxe_enabled"]).lower(),
            str(SETTINGS["pxe_menu_ipxe_timeout"]),
        )
        pm_ipxe["randomize_timeout"] = str(
            SETTINGS["pxe_menu_ipxe_randomize_timeout"]
        ).lower()
        pm_ipxe["prompt"], pm_ipxe["items"] = (
            SETTINGS["pxe_menu_ipxe_prompt"],
            SETTINGS["pxe_menu_ipxe_items"],
        )
        ...
        log_message(f"保存配置文件时出错: {e}", "ERROR")


def parse_dhcp_options(pkt_bytes):
    opts = {}
    if len(pkt_bytes) < 240 or pkt_bytes[236:240] != b"\x63\x82\x53\x63":
        return opts
    i = 240
    while i < len(pkt_bytes):
        ...
        length = pkt_bytes[i]
        i += 1
        if i + length > len(pkt_bytes):
            break
        value = pkt_bytes[i : i + length]
        i += length
        opts[code] = value
    return opts
...
def build_pxe_option43_menu(menu_cfg, tftp_server_ip):
    if not menu_cfg.get("enabled", False):
        return b""
    ...
    for line in menu_items_str.strip().splitlines():
        ...
        if len(parts) == 4:
            try:
                ...
                if server_type in seen_types:
                    continue
                seen_types.add(server_type)
                parsed_items.append(
                    {
                        "text": menu_text,
                        "type_bytes": server_type.to_bytes(2, "big"),
    ...
    for item in parsed_items:
        servers_val += item["type_bytes"] + b"\x01" + item["ip_bytes"]
...
def craft_dhcp_response(req_pkt, cfg, assigned_ip="0.0.0.0", is_proxy_req=False):
    ...
    try:
        ...
        log_message(f"DHCP: 解析请求包失败: {e}", "ERROR")
        return None
    ...
    if vendor_class:
        log_message(
            f"DHCP: 客户端 {client_mac} Vendor Class Identifier = '{vendor_class}'"
        )
    ...
    if "MSFT 5.0" in vendor_class:
        ...
        if client_manager:
            client_manager.handle_dhcp_request(
                client_mac, assigned_ip, "get_ip", hostname=hostname
            )
        ...
        if resp_msg_type == 0:
            return None
        resp_pkt = (
            bytearray(struct.pack("!BBBB", 2, 1, 6, 0))
            + xid
            + struct.pack("!HH", 0, 0x8000)
        )
        resp_pkt += req_pkt[12:16] + socket.inet_aton(assigned_ip) + b"\x00\x00\x00\x00"
        resp_pkt += (
            req_pkt[24:28] + chaddr + (b"\x00" * (64 + 128)) + b"\x63\x82\x53\x63"
        )
        resp_pkt += (
            bytes([53, 1, resp_msg_type])
            + bytes([54, 4])
            + socket.inet_aton(cfg["server_ip"])
        )
        ...
        if cfg.get("dhcp_options_enabled", False):
            custom_options_bytes = dhcp_option_handler.parse_and_build_dhcp_options(
                cfg.get("dhcp_options_text", "")
            )
    ...
    if client_manager:
        ...
        client_manager.handle_dhcp_request(
            client_mac, assigned_ip, initial_state, firmware_type=firmware_display
        )
    ...
    if is_ipxe_client:
        ...
        menu_cfg_key_prefix = (
            "pxe_menu_uefi" if "uefi" in arch_name else "pxe_menu_bios"
        )
    ...
    resp_msg_type = (
        5 if is_proxy_req else (2 if msg_type == 1 else (5 if msg_type == 3 else 0))
    )
    if resp_msg_type == 0:
        return None
    ...
    if selected_item_type is not None:
        if client_manager:
            client_manager.handle_dhcp_request(client_mac, assigned_ip, "menuselect")
                ...
                try:
                    if int(parts[2], 16) == selected_item_type:
                        ...
                        if "%dynamicboot%" in boot_file:
                            http_server_uri = (
                                f"http://{cfg['server_ip']}:{cfg['http_port']}"
                            )
                            replacement_string = (
                                f"{http_server_uri}/dynamic.ipxe?bootfile"
                            )
                            boot_file = boot_file.replace(
                                "%dynamicboot%", replacement_string
                            )
                    ...
                    continue
        log_message(
            f"DHCP: 客户端 {client_mac} 已选择菜单项 {selected_item_type:04x}, 提供文件: '{boot_file or '本地启动'}'"
        )
        option43 = (
            bytes([71, 4])
            + selected_item_type.to_bytes(2, "big")
            + (0).to_bytes(2, "big")
            + b"\xff"
        )

    elif menu_enabled and (
        not has_hostname or menu_cfg_key_prefix in ["pxe_menu_bios", "pxe_menu_uefi"]
    ):
        if is_ipxe_client:
            # <--- 修改点 2：修正这里的动态菜单 URL
            ...
            option43 = b""
            log_message(
                f"DHCP: 为iPXE客户端 {client_mac} 提供动态菜单脚本: '{boot_file}'"
            )
            if client_manager:
            ...
            menu_config = {
                "enabled": True,
                "timeout": cfg.get(f"{menu_cfg_key_prefix}_timeout", 10),
                "randomize_timeout": cfg.get(
                    f"{menu_cfg_key_prefix}_randomize_timeout", False
                ),
            ...
            log_message(
                f"DHCP: 为 {client_mac} ({arch_name.upper()}) 提供 '{menu_cfg_key_prefix}' 菜单"
            )
            if client_manager:
    ...
    if 97 in opts:
        resp_pkt += bytes([97, len(opts[97])]) + opts[97]
    if option43:
        resp_pkt += bytes([43, len(option43)]) + option43
            ...
            if len(parts) == 4:
                try:
                    if int(parts[2], 16) == selected_item_type:
                        ...
                        if "%dynamicboot%" in boot_file:
                            replacement_string = (
                                "http://${booturl}/dynamic.ipxe?bootfile"
                            )
                            boot_file = boot_file.replace(
                                "%dynamicboot%", replacement_string
                            )
...
dhcp_thread, proxy_thread, tftp_thread, http_thread, dhcp_detector_thread = (
    None,
    None,
    None,
    None,
...
def detect_other_dhcp_servers(stop_evt):
    ...
    try:
        while (
            not stop_evt.is_set() and time.time() - start_time < 15 and not found_server
        ):
            ...
            if detected_servers:
                for srv_ip in detected_servers:
                    log_message(
                        f"警告: 在局域网中发现另一个DHCP服务器, 地址为 {srv_ip}！",
                        "WARNING",
                    )
                log_message(
                    "建议: 为避免IP地址冲突, 请在本软件中使用“代理(Proxy)”模式, 或停用网络中的其它DHCP服务器。",
                    "WARNING",
                )
                found_server = True
...
def run_dhcp_server(cfg, stop_evt, client_mgr):
    ...
    if cfg["dhcp_mode"] == "dhcp":
        try:
            ...
            log_message(f"DHCP (67): IP池创建失败: {e}", "ERROR")
            return

    def get_lease_from_pool(mac):
        if mac in offered_ips and offered_ips[mac]["expires"] > time.time():
            return offered_ips[mac]["ip"]
        if not ip_pool:
            log_message("DHCP (67): IP池已耗尽!", "WARNING")
            return None
        ip = ip_pool.popleft()
        offered_ips[mac] = {"ip": ip, "expires": time.time() + 60}
        return ip

    def confirm_lease(mac, req_ip):
        if mac in mac_to_ip and mac_to_ip[mac] == req_ip:
            return req_ip
        if mac in offered_ips and offered_ips[mac]["ip"] == req_ip:
            mac_to_ip[mac] = req_ip
            del offered_ips[mac]
            return req_ip
    ...
    sock.settimeout(1.0)
    try:
        sock.bind((cfg["listen_ip"], 67))
        log_message(
            f"DHCP: 监听器已在 {cfg['listen_ip']}:67 启动 ({cfg['dhcp_mode']} 模式)"
        )
    except Exception as e:
        log_message(f"DHCP (67): 致命错误 - 无法绑定端口: {e}", "ERROR")
        return
        ...
        try:
            ...
            if cfg["dhcp_mode"] == "dhcp":
                ...
                if static_ip:
                    ...
                    log_message(
                        f"DHCP: 发现静态绑定: MAC {mac} -> IP {static_ip}", "INFO"
                    )
                else:
                    # 2. 如果没有静态绑定，执行动态分配逻辑
            ...
            if response_pkt:
                sock.sendto(response_pkt, ("255.255.255.255", 68))
        except socket.timeout:
            continue
        except Exception as e:
...
def run_proxy_listener(cfg, stop_evt):
    ...
    sock.settimeout(1.0)
    try:
        sock.bind((cfg["listen_ip"], 4011))
        log_message(f"ProxyDHCP: 监听器已在 {cfg['listen_ip']}:4011 启动")
    except Exception as e:
        log_message(f"ProxyDHCP (4011): 致命错误 - 无法绑定端口: {e}", "ERROR")
        return
    ...
    while not stop_evt.is_set():
        try:
            ...
            response_pkt = craft_dhcp_response(
                data, cfg, assigned_ip=client_ip_from_packet, is_proxy_req=True
            )
            if response_pkt:
                sock.sendto(response_pkt, addr)
        except socket.timeout:
            continue
        except Exception as e:
            log_message(f"ProxyDHCP (4011): 循环中发生错误: {e}", "ERROR")
    sock.close()
    log_message("ProxyDHCP (4011): 监听器已停止。")
        ...
        with aggregator_lock:
            ...
            for key, data in list(failure_aggregator.items()):
                ...
                if now - data["timestamp"] > 12:
                    ...
                    # 打印汇总后的唯一错误日志
                    log_message(
                        f"TFTP: [传输失败] 客户端 {ip} 请求 '{filename}' 多次超时 ({count}次尝试)",
                        "ERROR",
                    )
                    # 清理掉这个已报告的条目
...
def run_tftp_server(cfg, stop_evt):
    ...
    executor = (
        ThreadPoolExecutor(max_workers=max_transfers, thread_name_prefix="TFTP")
        if use_multithread
        else None
    )
    log_message(
        f"TFTP: 服务器已在 {cfg['listen_ip']}:69 启动 ({'多线程(最大' + str(max_transfers) + '线程)' if use_multithread else '单线程'}, 根目录: '{tftp_root}')"
    )
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as tsock:
                ...
                if opcode == 1:  # READ (下载)
                    ...
                    if not filepath.startswith(tftp_root) or not os.path.isfile(
                        filepath
                    ):
                        log_message(
                            f"TFTP: [拒绝] {client_addr} 请求了非法或不存在的文件 '{filename}'",
                            "WARNING",
                        )
                        tsock.sendto(
                            struct.pack("!HH", 5, 1) + b"File not found\x00",
                            client_addr,
                        )
                        return
                    ...
                    if is_modern_client:
                        ...
                        if b"blksize" in options:
                            try:
                                negotiated_blksize = max(
                                    512, min(int(options[b"blksize"]), 1428)
                                )
                                oack_parts.append(
                                    b"blksize\x00"
                                    + str(negotiated_blksize).encode()
                                    + b"\x00"
                                )
                            except (ValueError, IndexError):
                                pass
                        if b"tsize" in options:
                            oack_parts.append(
                                b"tsize\x00" + str(file_size).encode() + b"\x00"
                            )
                        ...
                        if oack_parts:
                            ...
                            try:
                                ...
                                if len(ack_data) >= 4:
                                    ack_opcode, ack_block_num = struct.unpack(
                                        "!HH", ack_data[:4]
                                    )
                                    if ack_opcode == 4 and ack_block_num == 0:
                                        log_message(
                                            f"TFTP: {client_addr} 已确认OACK (blksize={negotiated_blksize})。开始快速传输。",
                                            "INFO",
                                        )
                                        blksize = negotiated_blksize
                                    else:
                                        log_message(
                                            f"TFTP: {client_addr} 对OACK响应异常，回退至标准模式。",
                                            "WARNING",
                                        )
                                else:
                                    log_message(
                                        f"TFTP: {client_addr} 发送了无效的OACK确认，回退至标准模式。",
                                        "WARNING",
                                    )
                            except socket.timeout:
                                log_message(
                                    f"TFTP: {client_addr} 未确认OACK，回退至标准模式。",
                                    "INFO",
                                )
                    ...
                    with open(filepath, "rb") as f:
                        ...
                        while not stop_evt.is_set():
                            ...
                            for retry in range(5):
                                ...
                                log_message(
                                    f"TFTP: [传输失败] 等待 {client_addr} 对块 {block_num} 的ACK多次超时",
                                    "ERROR",
                                )
                                return
                            ...
                            if len(chunk) < blksize:
                                ...
                                log_message(
                                    f"TFTP: [成功] 文件 '{os.path.basename(filepath)}' -> {client_addr} 传输完成。"
                                )
                                break
                    ...
                    log_message(
                        f"TFTP: [WRITE] 收到来自 {client_addr} 对 '{filename}' 的上传请求。",
                        "INFO",
                    )
                    sanitized_filename = filename.replace("\\", "/").lstrip("/")
                    if not sanitized_filename or ".." in sanitized_filename.split("/"):
                        log_message(
                            f"TFTP: [拒绝] 收到来自 {client_addr} 的无效或恶意文件名 '{filename}'",
                            "WARNING",
                        )
                        tsock.sendto(
                            struct.pack("!HH", 5, 4) + b"Illegal TFTP operation\x00",
                            client_addr,
                        )
                        return
                    ...
                    if not os.path.realpath(filepath).startswith(
                        os.path.realpath(tftp_root)
                    ):
                        log_message(
                            f"TFTP: [拒绝] 检测到来自 {client_addr} 的目录遍历尝试 '{filename}'",
                            "WARNING",
                        )
                        tsock.sendto(
                            struct.pack("!HH", 5, 2) + b"Access violation\x00",
                            client_addr,
                        )
                        return
                    ...
                    if os.path.exists(filepath):
                        log_message(
                            f"TFTP: [警告] 文件 '{filepath}' 已存在。客户端 {client_addr} 即将覆盖该文件。",
                            "WARNING",
                        )
                    ...
                    try:
                        ...
                        log_message(
                            f"TFTP: [拒绝] 无法为 '{sanitized_filename}' 创建目录: {e}",
                            "ERROR",
                        )
                        tsock.sendto(
                            struct.pack("!HH", 5, 2) + b"Access violation\x00",
                    ...
                    with open(filepath, "wb") as f:
                        while True:
                            ...
                            if block_num == expected_block_num:
                                ...
                                tsock.sendto(
                                    struct.pack("!HH", 4, block_num), client_addr
                                )
                                expected_block_num = (expected_block_num + 1) % 65536
                                if len(chunk) < 512:
                                    log_message(
                                        f"TFTP: [写入成功] 文件 '{sanitized_filename}' ({total_bytes_written}字节) 已从 {client_addr} 接收完毕。"
                                    )
                                    transfer_successful = True
                                    break
            ...
            log_message(
                f"TFTP: 客户端 {client_addr} 已关闭连接 (可能传输已完成)。", "INFO"
            )
            transfer_successful = True
        except Exception as e:
            log_message(
                f"TFTP: 处理来自 {client_addr} 的请求时发生意外错误: {e}", "ERROR"
            )
        finally:
        ...
        while not stop_evt.is_set():
            try:
                ...
                if use_multithread and executor:
                    ...
                    threading.Thread(
                        target=handle_request, args=(data, addr), daemon=True
                    ).start()
            except socket.timeout:
                continue
    finally:
...
class RangeRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/dynamic.ipxe"):
            try:
                ...
                script_content = dynamic.generate_dynamic_script(
                    params=params, client_ip=client_ip
                )
        ...
        if os.path.isdir(fpath):
            log_message(
                f"HTTP: [目录浏览] 客户端 {self.client_address[0]} 正在浏览 '{self.path}'"
            )
            super().do_GET()
            return
        ...
        try:
            with open(fpath, "rb") as f:
                ...
                if not range_header:
                    ...
                    log_message(
                        f"HTTP: [200 GET] {self.path} -> {self.client_address[0]}"
                    )
                else:
                    ...
                    log_message(
                        f"HTTP: [206 Partial] {self.path} ({start}-{end}) -> {self.client_address[0]}"
                    )
...
def run_http_server(cfg, stop_evt):
    ...
    Handler = functools.partial(
        DirectoryAwareHandler, client_manager_instance=client_manager
    )

    use_multithread = cfg.get("http_multithread", True)
    http_server_class = (
        ThreadPoolTCPServer if use_multithread else socketserver.TCPServer
    )
    ...
    try:
        with http_server_class((cfg["listen_ip"], cfg["http_port"]), Handler) as httpd:
            log_message(
                f"HTTP: 服务器已在 http://{cfg['server_ip']}:{cfg['http_port']}/ 启动 ({'多线程' if use_multithread else '单线程'}, 根目录: {http_root_dir})"
            )
...
def manage_smb_share(settings, start=True):
    ...
    share_name, share_path = (
        settings.get("smb_share_name"),
        os.path.realpath(settings.get("smb_root")),
    )
    ...
    try:
        ...
        log_message("SMB: 'net' 命令未找到。", "ERROR")
        return
    if not start:
        log_message(f"SMB: 共享 '{share_name}' 已停止。")
        return
    if settings.get("smb_enabled"):
        if not os.path.exists(share_path):
            try:
                os.makedirs(share_path)
                log_message(f"SMB: 已创建根目录 '{share_path}'")
            except OSError as e:
                log_message(f"SMB: 创建根目录失败: {e}", "ERROR")
                return
        try:
            permissions = (
                "/grant:Everyone,FULL"
                if settings.get("smb_permissions") == "full"
                else "/grant:Everyone,READ"
            )
            ...
            log_message(
                f"SMB: 已将 '{share_path}' 共享为 '{share_name}' ({perm_text})。访问路径: \\\\{settings['server_ip']}\\{share_name}"
            )
        except subprocess.CalledProcessError as e:
            log_message(
                f"SMB: 创建共享失败。请以管理员身份运行。详情: {e.stderr.strip()}",
                "ERROR",
            )


def is_smb_share_active(share_name):
    if os.name != "nt" or not SETTINGS.get("smb_enabled"):
        return False
    try:
...
def start_services():
    ...
    dynamic.initialize_dynamic_scripting(
        current_settings, client_manager_instance=client_manager
    )
    ...
    if current_settings["dhcp_enabled"]:
        ...
        dhcp_thread = threading.Thread(
            target=run_dhcp_server,
            args=(current_settings, stop_event, client_manager),
            daemon=True,
        )
        dhcp_thread.start()
        proxy_thread = threading.Thread(
            target=run_proxy_listener, args=(current_settings, stop_event), daemon=True
        )
        proxy_thread.start()
        if current_settings["dhcp_mode"] == "dhcp":
            dhcp_detector_thread = threading.Thread(
                target=detect_other_dhcp_servers, args=(stop_event,), daemon=True
            )
            dhcp_detector_thread.start()
    if current_settings["tftp_enabled"]:
        tftp_thread = threading.Thread(
            target=run_tftp_server, args=(current_settings, stop_event), daemon=True
        )
        tftp_thread.start()
    if current_settings["http_enabled"]:
        http_thread = threading.Thread(
            target=run_http_server, args=(current_settings, stop_event), daemon=True
        )
        http_thread.start()
...
class ConfigWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        dhcp_option_handler.create_dhcp_options_tab(
            dhcp_options_frame, self.settings_vars, SETTINGS
        )
        ...
        ttk.Button(button_frame, text="保存并关闭", command=self.save_and_close).pack(
            side="right", padx=5
        )
        ttk.Button(button_frame, text="取消", command=self.destroy).pack(side="right")

    def _enforce_ipxe_menu_dependency(self, *args):
        is_bios_enabled = self.settings_vars.get(
            "pxe_menu_bios_enabled", tk.BooleanVar(value=False)
        ).get()
        is_uefi_enabled = self.settings_vars.get(
            "pxe_menu_uefi_enabled", tk.BooleanVar(value=False)
        ).get()
    ...
    def _recalculate_dhcp_pool(self, basis_ip):
        try:
            ...
            if "ip_pool_start" in self.settings_vars:
                self.settings_vars["ip_pool_start"].set(pool_start)
            if "ip_pool_end" in self.settings_vars:
                self.settings_vars["ip_pool_end"].set(pool_end)
            if "router_ip" in self.settings_vars:
                self.settings_vars["router_ip"].set(router)
            if "dns_server_ip" in self.settings_vars:
                self.settings_vars["dns_server_ip"].set(dns)
        except (ValueError, IndexError):
            pass
    ...
    def create_general_tab(self, parent):
        ...
        ttk.Label(parent, text="监听IP地址:").grid(
            row=0, column=0, sticky="w", padx=5, pady=5
        )
        ips = get_all_ips()
        ip_options = ["0.0.0.0 (所有网卡)"] + ips
        self.settings_vars["listen_ip"] = tk.StringVar(value=SETTINGS.get("listen_ip"))
        ip_combo = ttk.Combobox(
            parent, textvariable=self.settings_vars["listen_ip"], values=ip_options
        )
        ...
        ttk.Label(parent, text="本机服务器IP:").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        self.settings_vars["server_ip"] = tk.StringVar(value=SETTINGS.get("server_ip"))
        server_ip_combo = ttk.Combobox(
            parent, textvariable=self.settings_vars["server_ip"], values=ips
        )
        server_ip_combo.grid(row=1, column=1, columnspan=3, sticky="ew", padx=5, pady=5)

        ttk.Label(parent, text="(用于通告给客户端，如TFTP Server IP)").grid(
            row=2, column=1, sticky="w", padx=5, pady=2, columnspan=3
        )
        ttk.Separator(parent, orient="horizontal").grid(
            row=3, column=0, columnspan=4, sticky="ew", pady=15
        )
        self.settings_vars["dhcp_enabled"] = tk.BooleanVar(
            value=SETTINGS.get("dhcp_enabled")
        )
        dhcp_enabled_check = ttk.Checkbutton(
            parent,
            text="启用 DHCP 服务",
            variable=self.settings_vars["dhcp_enabled"],
            command=self.toggle_dhcp_controls,
        )
        ...
        self.dhcp_radio = ttk.Radiobutton(
            self.radio_frame,
            text="DHCP (完整模式)",
            variable=self.settings_vars["dhcp_mode"],
            value="dhcp",
        )
        self.dhcp_radio.pack(side="left")
        self.proxy_radio = ttk.Radiobutton(
            self.radio_frame,
            text="Proxy (代理模式)",
            variable=self.settings_vars["dhcp_mode"],
            value="proxy",
        ...
        self.dhcp_settings_frame = ttk.LabelFrame(
            parent, text="DHCP (完整模式) 设置", padding="10"
        )
        self.dhcp_settings_frame.grid(
            row=6, column=0, columnspan=4, sticky="ew", pady=(10, 0), padx=2
        )
        self.dhcp_settings_frame.columnconfigure(1, weight=1)
        self.dhcp_settings_frame.columnconfigure(3, weight=1)
        fields = [
            ("地址池起始:", "ip_pool_start", 0, 0),
            ("地址池结束:", "ip_pool_end", 0, 2),
            ("子网掩码 (Opt 1):", "subnet_mask", 1, 0),
            ("租约 (秒, Opt 51):", "lease_time", 1, 2),
            ("网关 (Opt 3):", "router_ip", 2, 0),
            ("DNS (Opt 6):", "dns_server_ip", 2, 2),
        ]
        for label, key, r, c in fields:
            ttk.Label(self.dhcp_settings_frame, text=label).grid(
                row=r, column=c, sticky="w", pady=2, padx=5
            )
            VarClass = tk.IntVar if "lease_time" in key else tk.StringVar
            self.settings_vars[key] = VarClass(value=SETTINGS.get(key))
            ttk.Entry(
                self.dhcp_settings_frame, textvariable=self.settings_vars[key]
            ).grid(row=r, column=c + 1, sticky="ew", pady=2, padx=5)
        self.settings_vars["dhcp_mode"].trace_add("write", self.toggle_dhcp_fields)
        self.toggle_dhcp_controls()
    ...
    def create_path_tab(self, parent):
        ...
        self.settings_vars["tftp_enabled"] = tk.BooleanVar(
            value=SETTINGS.get("tftp_enabled")
        )
        ttk.Checkbutton(
            tftp_frame,
            text="启用 TFTP 服务",
            variable=self.settings_vars["tftp_enabled"],
        ).pack(side="left")
        self.settings_vars["tftp_multithread"] = tk.BooleanVar(
            value=SETTINGS.get("tftp_multithread")
        )
        ttk.Checkbutton(
            tftp_frame,
            text="多线程处理",
            variable=self.settings_vars["tftp_multithread"],
        ).pack(side="left", padx=(15, 0))
        ttk.Label(tftp_frame, text="最大线程:").pack(side="left", padx=(15, 0))
        self.settings_vars["tftp_max_transfers"] = tk.IntVar(
            value=SETTINGS.get("tftp_max_transfers", 60)
        )
        ttk.Entry(
            tftp_frame, textvariable=self.settings_vars["tftp_max_transfers"], width=6
        ...
        ttk.Entry(parent, textvariable=self.settings_vars["tftp_root"]).grid(
            row=1, column=1, sticky="ew", pady=5
        )
        ttk.Button(
            parent,
            text="浏览...",
            command=lambda: browse_directory(self.settings_vars["tftp_root"]),
        ).grid(row=1, column=2, padx=5, pady=5)
        ttk.Separator(parent).grid(row=2, columnspan=3, sticky="ew", pady=10)
        http_frame = ttk.Frame(parent)
        http_frame.grid(row=3, column=0, columnspan=3, sticky="ew")
        self.settings_vars["http_enabled"] = tk.BooleanVar(
            value=SETTINGS.get("http_enabled")
        )
        ttk.Checkbutton(
            http_frame,
            text="启用 HTTP 服务",
            variable=self.settings_vars["http_enabled"],
        ).pack(side="left")
        self.settings_vars["http_multithread"] = tk.BooleanVar(
            value=SETTINGS.get("http_multithread")
        )
        ttk.Checkbutton(
            http_frame,
            text="多线程处理",
            variable=self.settings_vars["http_multithread"],
        ).pack(side="left", padx=(15, 0))
        ...
        ttk.Entry(parent, textvariable=self.settings_vars["http_port"], width=10).grid(
        ...
        ttk.Entry(parent, textvariable=self.settings_vars["http_root"]).grid(
            row=5, column=1, sticky="ew", pady=5
        )
        ttk.Button(
            parent,
            text="浏览...",
            command=lambda: browse_directory(self.settings_vars["http_root"]),
        ).grid(row=5, column=2, padx=5, pady=5)
        ttk.Separator(parent).grid(row=6, columnspan=3, sticky="ew", pady=10)
        self.settings_vars["smb_enabled"] = tk.BooleanVar(
            value=SETTINGS.get("smb_enabled")
        )
        ttk.Checkbutton(
            parent,
            text="启用 SMB 文件共享 (仅Windows)",
            variable=self.settings_vars["smb_enabled"],
        ).grid(row=7, column=0, columnspan=3, sticky="w")
        ttk.Label(parent, text="SMB 共享名称:").grid(
            row=8, column=0, sticky="w", pady=5
        )
        self.settings_vars["smb_share_name"] = tk.StringVar(
            value=SETTINGS.get("smb_share_name")
        )
        ttk.Entry(parent, textvariable=self.settings_vars["smb_share_name"]).grid(
            row=8, column=1, sticky="ew", pady=5
        )
        ttk.Label(parent, text="共享权限:").grid(row=9, column=0, sticky="w", pady=5)
        self.settings_vars["smb_permissions"] = tk.StringVar(
            value=SETTINGS.get("smb_permissions", "read")
        )
        smb_perm_frame = ttk.Frame(parent)
        smb_perm_frame.grid(row=9, column=1, sticky="w")
        ttk.Radiobutton(
            smb_perm_frame,
            text="只读",
            variable=self.settings_vars["smb_permissions"],
            value="read",
        ).pack(side="left")
        ttk.Radiobutton(
            smb_perm_frame,
        ...
        ttk.Entry(parent, textvariable=self.settings_vars["smb_root"]).grid(
            row=10, column=1, sticky="ew", pady=5
        )
        ttk.Button(
            parent,
            text="浏览...",
            command=lambda: browse_directory(self.settings_vars["smb_root"]),
        ).grid(row=10, column=2, padx=5, pady=5)
        ttk.Separator(parent).grid(row=11, columnspan=3, sticky="ew", pady=15)
        ttk.Button(parent, text="一键设置为当前目录", command=set_all_paths).grid(
            row=12, column=1, sticky="w", pady=10
        )
...
当对应客户端类型 (BIOS, UEFI, 或 iPXE) 的“菜单”功能被禁用时，服务器将提供这里指定的文件。"""
        ttk.Label(parent, text=new_description, wraplength=500, justify=tk.LEFT).grid(
            row=0, column=0, columnspan=3, sticky="w", pady=(5, 15), padx=5
        )
        files_map = [
            ("BIOS 启动文件:", "bootfile_bios"),
            ("UEFI32 启动文件:", "bootfile_uefi32"),
            ("UEFI64 启动文件:", "bootfile_uefi64"),
            ("iPXE 脚本文件:", "bootfile_ipxe"),
        ]
        for i, (label, key) in enumerate(files_map):
            ...
            ttk.Entry(parent, textvariable=self.settings_vars[key]).grid(
                row=i + 1, column=1, sticky="ew", pady=5, padx=5
            )
    ...
    def create_pxe_menu_tab(self, parent, arch_type):
        ...
        menu_check = ttk.Checkbutton(
            check_frame, text=menu_check_text, variable=self.settings_vars[enabled_key]
        )
        ...
        if arch_type in ["bios", "uefi"]:
            ...
            ttk.Label(
                check_frame, text="(依赖iPXE菜单，将自动启用)", foreground="grey"
            ).pack(side="left", padx=5)

        pxe_menu_frame = ttk.LabelFrame(parent, text="菜单设置", padding="10")
        pxe_menu_frame.grid(row=1, column=0, columnspan=2, sticky="ew", pady=5)
        pxe_menu_frame.columnconfigure(1, weight=1)
        prompt_key, timeout_key, items_key = (
            f"pxe_menu_{arch_type}_prompt",
            f"pxe_menu_{arch_type}_timeout",
            f"pxe_menu_{arch_type}_items",
        )
        ttk.Label(pxe_menu_frame, text="菜单提示文本:").grid(
            row=0, column=0, sticky="w", pady=5
        )
        self.settings_vars[prompt_key] = tk.StringVar(value=SETTINGS.get(prompt_key))
        ttk.Entry(pxe_menu_frame, textvariable=self.settings_vars[prompt_key]).grid(
            row=0, column=1, sticky="ew", pady=5
        )
        ttk.Label(pxe_menu_frame, text="菜单超时 (秒):").grid(
        ...
        ttk.Entry(
            timeout_frame, textvariable=self.settings_vars[timeout_key], width=10
        ).pack(side="left")
        randomize_key = f"pxe_menu_{arch_type}_randomize_timeout"
        self.settings_vars[randomize_key] = tk.BooleanVar(
            value=SETTINGS.get(randomize_key)
        )
        ttk.Checkbutton(
            timeout_frame,
            text="客户机时间随机分配",
            variable=self.settings_vars[randomize_key],
        ).pack(side="left", padx=(10, 0))
        ...
        menu_items_text = scrolledtext.ScrolledText(
            items_frame, wrap=tk.WORD, height=10, width=60
        )
        menu_items_text.pack(fill="both", expand=True)
        menu_items_text.insert(tk.END, SETTINGS.get(items_key, ""))
        self.settings_vars[items_key + "_widget"] = menu_items_text
        ttk.Label(
            pxe_menu_frame,
            text="格式: 菜单文本, 启动文件, 类型(4位Hex), 服务器IP",
            justify=tk.LEFT,
            foreground="grey",
    ...
    def toggle_dhcp_controls(self, *args):
        ...
        widgets_to_toggle = [
            self.dhcp_mode_label,
            self.dhcp_radio,
            self.proxy_radio,
            self.dhcp_settings_frame,
        ]
        self.toggle_widget_state(widgets_to_toggle, state)
        if is_enabled:
            self.toggle_dhcp_fields()
        else:
            self.toggle_widget_state(
                self.dhcp_settings_frame.winfo_children(), "disabled"
            )
    ...
    def save_and_close(self):
        ...
        for key, var in self.settings_vars.items():
            if key.endswith("_widget"):
                widget = var
                base_key = key.replace("_widget", "")
                temp_settings[base_key] = widget.get("1.0", tk.END).strip()
            else:
                try:
                    val = var.get()
                    if key == "listen_ip" and val == "0.0.0.0 (所有网卡)":
                        val = "0.0.0.0"
                    temp_settings[key] = val
                except (tk.TclError, ValueError):
                    temp_settings[key] = 0 if isinstance(var, tk.IntVar) else ""
...
class NBpxeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("NBPXE 服务器 20260505")
        ...
        client_list_frame = ttk.LabelFrame(
            paned_window, text="客户端列表", padding="10"
        )
        global client_manager
        client_manager = ClientManager(
            client_list_frame, log_message, settings=SETTINGS
        )  # Pass settings
        client_manager.pack(fill="both", expand=True)
        log_frame = ttk.LabelFrame(paned_window, text="实时日志", padding="10")
        self.log_text = scrolledtext.ScrolledText(
            log_frame, wrap=tk.WORD, state="disabled", height=5
        )
        self.log_text.pack(fill="both", expand=True)
        self.log_text.tag_config(
            "warning", foreground="orange", font=("Helvetica", 9, "bold")
        )
        self.log_text.tag_config(
            "error", foreground="red", font=("Helvetica", 9, "bold")
        )
    ...
    def create_status_widgets(self, parent):
        ...
        for i, service in enumerate(services):
            ttk.Label(parent, text=f"{service}:").grid(
                row=0, column=i * 2, sticky="w", padx=(0, 5)
            )
            self.status_labels[service] = ttk.Label(
                parent, text="■ 已停止", foreground="red", font=("Helvetica", 9, "bold")
            )
            self.status_labels[service].grid(
                row=0, column=i * 2 + 1, sticky="w", padx=(0, 20)
            )

    def create_control_widgets(self, parent):
        ttk.Button(parent, text="启动 / 重启服务", command=start_services).pack(
            side="left", padx=5, pady=5
        )
        ttk.Button(parent, text="停止所有服务", command=stop_services).pack(
            side="left", padx=5, pady=5
        )
        ttk.Button(
            parent, text="修改配置", command=lambda: ConfigWindow(self.root)
        ).pack(side="left", padx=5, pady=5)
        ttk.Button(parent, text="重置配置文件", command=self.reset_ini_file).pack(
            side="left", padx=(15, 5), pady=5
        )
        ttk.Button(parent, text="退出程序", command=self.on_closing).pack(
    ...
    def reset_ini_file(self):
        if messagebox.askyesno(
            ...
            try:
                ...
                messagebox.showinfo(
                    "重置成功",
                    f"配置文件 '{INI_FILENAME}' 已被删除。\n程序即将关闭，请手动重启。",
                )
                self.on_closing(force=True)
            except Exception as e:
                messagebox.showerror(
                    "删除失败", f"无法删除配置文件 '{INI_FILENAME}': {e}"
                )
    ...
    def update_status_display(self):
        ...
        if SETTINGS.get("dhcp_enabled"):
            if is_dhcp_running and is_proxy_running:
                ...
                self.status_labels["DHCP"].config(
                    text=f"● 运行中 ({mode})", foreground="green"
                )
            else:
                self.status_labels["DHCP"].config(text="■ 未启动", foreground="orange")
        else:
            self.status_labels["DHCP"].config(text="■ 已禁用", foreground="grey")
        is_tftp_running = tftp_thread and tftp_thread.is_alive()
        if SETTINGS.get("tftp_enabled"):
            text, color = (
                ("● 运行中", "green") if is_tftp_running else ("■ 未启动", "orange")
            )
            self.status_labels["TFTP"].config(text=text, foreground=color)
        else:
            self.status_labels["TFTP"].config(text="■ 已禁用", foreground="grey")
        is_http_running = http_thread and http_thread.is_alive()
        if SETTINGS.get("http_enabled"):
            text, color = (
                ("● 运行中", "green") if is_http_running else ("■ 未启动", "orange")
            )
            self.status_labels["HTTP"].config(text=text, foreground=color)
        else:
            self.status_labels["HTTP"].config(text="■ 已禁用", foreground="grey")
        if SETTINGS.get("smb_enabled"):
            if is_smb_share_active(SETTINGS.get("smb_share_name")):
                self.status_labels["SMB"].config(text="● 共享中", foreground="green")
            else:
                self.status_labels["SMB"].config(text="■ 未共享", foreground="orange")
        else:
            self.status_labels["SMB"].config(text="■ 已禁用", foreground="grey")
